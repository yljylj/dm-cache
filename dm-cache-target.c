...
#define DBG(m, n, p...) \
	if (bio_flagged(m, OWN)) \
		printk(KERN_INFO n, ##p)
...
{
	//DBG(KERN_INFO "is_dirty\n");
	return test_bit(from_cblock(b), cache->dirty_bitset);
}

static void set_dirty(struct cache *cache, dm_cblock_t cblock)
{
	//DBG(KERN_INFO "set_dirty\n");
	if (!test_and_set_bit(from_cblock(cblock), cache->dirty_bitset)) {
		printk(KERN_INFO "sync  %s", current->comm);
...
{
	//DBG(KERN_INFO "force_set_dirty\n");
	if (!test_and_set_bit(from_cblock(cblock), cache->dirty_bitset)){
		atomic_inc(&cache->nr_dirty);
		printk(KERN_INFO "not sync");
	}
	policy_set_dirty(cache->policy, cblock);
...
{
	//DBG(KERN_INFO "force_clear_dirty\n");
				 ...
				 dm_oblock_t oblock, dm_cblock_t cblock)
{
	...
	if (bio_data_dir(bio) == WRITE) {
		...
		//printk(KERN_INFO "remap_to_cache_dirty\n");
		clear_discard(cache, oblock_to_dblock(cache, oblock));
	}
}
...
static void process_deferred_bios(struct work_struct *ws)
{
	...
	struct bio_list bios;
	struct bio *bio;
        //DBG(KERN_INFO "process_deferred_bios\n");
	bio_list_init(&bios);
...
static void requeue_deferred_bios(struct cache *cache)
{
	struct bio *bio;
	struct bio_list bios;
        //DBG(KERN_INFO "requeue_deferred_bios\n");
	bio_list_init(&bios);
	bio_list_merge(&bios, &cache->deferred_bios);
	bio_list_init(&cache->deferred_bios);
...
static int cache_end_io(struct dm_target *ti, struct bio *bio, int error)
{
	...
	size_t pb_data_size = get_per_bio_data_size(cache);
	struct per_bio_data *pb = get_per_bio_data(bio, pb_data_size);
        //printk(KERN_INFO "cache_end_io\n");
        DBG(bio, "cache_end_io\n");
	if (pb->tick) {
...
{
	int r;
	//printk(KERN_INFO "write_dirty_bitset\n");
	if (get_cache_mode(cache) >= CM_READ_ONLY)
		return -EINVAL;
...
{
	unsigned i, r;
	//printk(KERN_INFO "write_discard_bitset\n");
	if (get_cache_mode(cache) >= CM_READ_ONLY)
		return -EINVAL;
...
static int resize_cache_dev(struct cache *cache, dm_cblock_t new_size)
{
	...
	r = dm_cache_resize(cache->cmd, new_size);
        //printk(KERN_INFO "resize_cache_devi\n");
	if (r) {
...
{
	int r = 0;
        //printk(KERN_INFO "cache_preresume\n");
	struct cache *cache = ti->private;
...
{
        //printk(KERN_INFO "cache_bvec_merge\n");
	struct cache *cache = ti->private;
...
static void set_discard_limits(struct cache *cache, struct queue_limits *limits)
{
	 ...
	 */
        //printk(KERN_INFO "set_discard_limits\n");
...
static void cache_io_hints(struct dm_target *ti, struct queue_limits *limits)
{
	...
	uint64_t io_opt_sectors = limits->io_opt >> SECTOR_SHIFT;
        //printk(KERN_INFO "cache_io_hint\n");
	/*
